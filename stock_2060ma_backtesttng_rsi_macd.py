...
def calculate_ma_signals(stock_data):
    ...
    for i in range(200, len(df)):
        ...
        # MACD条件：MACD在信号线之上或刚刚金叉
        macd_bullish = (macd > macd_signal) or (macd > 0 and macd_signal > 0)

        # RSI条件：RSI在合理区间（30-70），避免超买超卖
        rsi_ok = (rsi > 30) and (rsi < 70)

        # 买入条件：5MA与20MA黄金交叉且在200MA之上 + MACD和RSI过滤 + 未持仓
        if (golden_cross_5_20 and close_price > ma200 and
                macd_bullish and rsi_ok and not holding):
...
def plot_technical_analysis(stock_data, buy_signals, sell_signals, stack_code, cumulative_returns=None):
    ...
    ax1.set_title(f'台积电({stack_code}) - 股价与移动平均线 (5MA/20MA交叉策略 + MACD/RSI过滤)', fontsize=14)
...
1. 5MA与20MA黄金交叉
2. 收盘价在200MA之上
3. MACD在信号线之上或双线在零轴之上
4. RSI在30-70之间（避免超买超卖）

卖出条件：
1. 20MA与60MA死亡交叉

技术指标说明：
- MACD: 趋势动量指标，金叉看涨，死叉看跌
- RSI: 相对强弱指标，30以下超卖，70以上超买
        """
        ax6.text(0.05, 0.95, strategy_info, transform=ax6.transAxes, fontsize=12,
...
def main():
    ...
    try:
        ...
        # 计算移动平均线和信号（包含MACD和RSI过滤）
        stock_with_indicators, buy_signals, sell_signals = calculate_ma_signals(stock)
