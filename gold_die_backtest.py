...
def simple_stock_analysis():
    ...
    for date, row in sell_signals.iterrows():
        ...
        plt.annotate('S', xy=(date, high_value * 1.02), xytext=(0, -10),
                     ...
                     color='red', fontweight='bold')  # 修复这里的语法错误
    ...
    # 计算策略相对于买入持有的超额收益
    excess_return = backtest_results['total_return'] - backtest_results['buy_hold_return']
    excess_annual = backtest_results['annual_return'] - backtest_results['buy_hold_annual']
    ...
    plt.scatter(sell_dates, sell_values, marker='v', color='red', s=100, label='卖出点', zorder=5)

    plt.title(f'{stock_code} 移动平均线交叉策略 vs 买入持有策略 资产曲线对比')
    plt.xlabel('日期')
    plt.ylabel('资产价值 (元)')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
